...
static bool process_file(const char *input_path, const doc_options_t *options, cli_context_t *ctx) {
  if (ctx->verbose) {
...
static bool process_directory(const char *input_dir, const doc_options_t *options, cli_context_t *ctx) {
...
static bool generate_index_markdown(const char *output_dir, const doc_options_t *options, cli_context_t *ctx) {
  ...
  fprintf(f, "This documentation was generated automatically from Xylia source files.\n\n");
  fprintf(f, "## Modules\n\n");
  ...
  if (dir) {
    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      if (ext && strcmp(ext, ".md") == 0 && strcmp(entry->d_name, "index.md") != 0) {
...
static bool generate_index_html(const char *output_dir, const doc_options_t *options, cli_context_t *ctx) {
  char index_path[1024];
  ...
  fprintf(f, "        .modules { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px; }\n");
  fprintf(f, "        .module-card { border: 1px solid #e1e5e9; border-radius: 6px; padding: 20px; background: #fff; }\n");
  fprintf(f, "        .module-card:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.1); }\n");
  fprintf(f, "        .module-card h3 { margin: 0 0 10px 0; color: #0366d6; }\n");
  fprintf(f, "        .module-card a { text-decoration: none; color: inherit; }\n");
  ...
  fprintf(f, "            <h1>%s</h1>\n", options->title ? options->title : "Documentation");
  fprintf(f, "            <p>This documentation was generated automatically from Xylia source files.</p>\n");
  fprintf(f, "        </div>\n");
  ...
  if (dir) {
    ...
    while ((entry = readdir(dir)) != NULL) {
      ...
      const char *ext = strrchr(entry->d_name, '.');
      if (ext && strcmp(ext, ".html") == 0 && strcmp(entry->d_name, "index.html") != 0) {
...
static bool generate_index(const char *output_dir, const doc_options_t *options, cli_context_t *ctx) {
...
static void show_docs_help(void) {
  ...
  printf("  -f, --format FORMAT  Output format: markdown, html (default: markdown)\n");
  ...
  printf("Examples:\n");
  printf("  xylia docs main.xyl                 # Generate docs for single file\n");
  printf("  xylia docs src/                     # Generate docs for directory\n");
  printf("  xylia docs -o ./output src/         # Specify output directory\n");
  printf("  xylia docs -f html --title \"My Docs\" src/  # HTML format with title\n");
}
...
cli_result_t cli_docs(int argc, char **argv, cli_context_t *ctx) {
  ...
  if (ctx->verbose) {
    ...
    printf("Format: %s\n", 
           ...
           options.format == DOC_FORMAT_HTML ? "html" : "json");
  }
